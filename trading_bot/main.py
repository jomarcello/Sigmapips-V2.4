...
try:
    ...
    print("Error importing sys module. Make sure sys is available in your Python environment.")
    os.environ['PYTHONPATH'] = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    print(f"Set PYTHONPATH environment variable instead: {os.environ['PYTHONPATH']}")
...
try:
    # First try the absolute import (works when PYTHONPATH is set correctly)
    ...
    try:
        # Try relative import (works when run as a module)
        from .services.telegram_service.states import (
            MENU, ANALYSIS, SIGNALS, CHOOSE_MARKET, CHOOSE_INSTRUMENT, CHOOSE_STYLE,
            CHOOSE_ANALYSIS, SIGNAL_DETAILS,
            CALLBACK_MENU_ANALYSE, CALLBACK_MENU_SIGNALS, CALLBACK_ANALYSIS_TECHNICAL,
            CALLBACK_ANALYSIS_SENTIMENT, CALLBACK_ANALYSIS_CALENDAR, CALLBACK_SIGNALS_ADD,
        ...
        # Fallback to direct path-based import
        import sys
        import os
        sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
        from trading_bot.services.telegram_service.states import (
...
from trading_bot.services.admin_service import AdminService
from trading_bot.services.cache_service import CacheService
# Fix circular import issue by using getter function instead of direct import
from trading_bot.services.chart_service import get_chart_service
from trading_bot.services.localization_service.translator import translate
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            from services.telegram_service.gif_utils import send_loading_gif
...
<b>Start today with a FREE 14-day trial!</b>
""" # Correctly close the multiline f-string
        
        # Create keyboard with subscription button
        keyboard = [
            [InlineKeyboardButton("🔄 Subscribe", url=subscription_url)]
        ]
...
if __name__ == "__main__":
    ...
    # Check if root logger already has handlers to prevent duplicates
    if not logging.getLogger().hasHandlers():
        logging.basicConfig(
            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
            level=logging.INFO
        )
    logger = logging.getLogger(__name__) # Define logger at this scope

    async def run_main_bot():
        try:
            logger.info("Attempting to start bot from main.py entry point...")
            bot_token = os.getenv("TELEGRAM_BOT_TOKEN")
            if not bot_token:
                logger.error("TELEGRAM_BOT_TOKEN environment variable not set.")
                sys.exit(1)

            # Initialize dependencies required by TelegramService
            logger.info("Initializing Database...")
            db = Database() # Assuming Database doesn't need async init
            logger.info("Initializing StripeService...")
            stripe_service = StripeService() # Assuming StripeService doesn't need async init
            ...
            # Initialize async components within TelegramService
            logger.info("Initializing TelegramService async components...")
            await telegram_service.initialize_services()
            
            # Determine how to run the application (Polling or Webhook)
            ...
            if force_polling or not webhook_url:
                logger.info("Starting bot in polling mode from main.py")
                ...
                logger.info(f"Starting bot with webhook from main.py. URL: {full_webhook_url}, Port: {port}, Path: {webhook_path}")
                await telegram_service.application.run_webhook(
                    listen="0.0.0.0",
                    port=port,
             ...
             logger.error(f"ImportError starting bot from main.py: {e}. Make sure all dependencies are installed and PYTHONPATH is correct.")
             logger.exception(e)
             sys.exit(1)
        except Exception as e:
            logger.error(f"Generic error starting bot from main.py: {e}")
            logger.exception(e)
            sys.exit(1)
