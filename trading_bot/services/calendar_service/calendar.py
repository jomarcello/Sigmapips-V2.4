...
class EconomicCalendarService:
    ...
    async def _get_economic_calendar_data(self, currency_list, start_date, end_date, lookback_hours = 8):
        ...
        """
        try:
            # Skip Tavily API initialization since it's no longer used
            logger.info(f"Economic calendar requested for currencies: {currency_list}")
            logger.info(f"Using mock calendar data (Tavily not used)")
            
            # Directly return mock calendar data
