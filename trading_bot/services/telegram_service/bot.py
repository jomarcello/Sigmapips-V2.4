...
# Use lazy import for ChartService to avoid circular dependency
# from trading_bot.services.chart_service.chart import ChartService
try:
    from trading_bot.services.sentiment_service.sentiment import MarketSentimentService
    logger.info("MarketSentimentService imported successfully from sentiment.py")
except ImportError as e:
    logger.warning(f"Failed to import MarketSentimentService from sentiment.py: {str(e)}")
    try:
        # Fallback to the sentiment class from __init__.py if specific module is not available
        from trading_bot.services.sentiment_service import MarketSentimentService
        logger.info("MarketSentimentService imported successfully from __init__.py")
    except ImportError as e:
        logger.error(f"Failed to import MarketSentimentService from __init__.py: {str(e)}")
        # Create a minimal implementation if needed
        class MarketSentimentService:
            """Minimal implementation of MarketSentimentService when actual module is unavailable"""
            def __init__(self, *args, **kwargs):
                logger.warning("Using minimal implementation of MarketSentimentService")
            
            async def get_sentiment(self, instrument, market_type=None):
                return {"analysis": f"<b>⚠️ Sentiment analysis unavailable</b>\n\nThe sentiment analysis service is currently unavailable."}
        logger.info("Created minimal implementation of MarketSentimentService")

# Try to import EconomicCalendarService with fallback
try:
    from trading_bot.services.calendar_service import EconomicCalendarService
    logger.info("EconomicCalendarService imported successfully")
except ImportError as e:
    logger.error(f"Failed to import EconomicCalendarService: {str(e)}")
    # Create minimal implementation
    class EconomicCalendarService:
        """Minimal implementation of EconomicCalendarService when actual module is unavailable"""
        def __init__(self, *args, **kwargs):
            logger.warning("Using minimal implementation of EconomicCalendarService")
        
        async def get_economic_calendar(self, currencies=None, days_ahead=0, min_impact="Low"):
            return "<b>📅 Economic Calendar</b>\n\nCalendar service is currently unavailable."
        
        async def get_instrument_calendar(self, instrument, *args, **kwargs):
            return "<b>📅 Economic Calendar</b>\n\nCalendar service is currently unavailable."
    logger.info("Created minimal implementation of EconomicCalendarService")
...
try:
    # First try the relative import (safest in this context)
    from .states import (
...
# Note about Tavily not being used
logger.info("Tavily API service is disabled - using mock data")
TAVILY_API_KEY = "tavily-disabled"
...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def menu_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE = None) -> None:
        ...
        try:
            ...
            logger.info(f"Update object content: {update}")
            ...
            if update and update.effective_chat:
                ...
                # Fallback method to get chat_id
                try:
                    if hasattr(update, 'message') and update.message:
                        chat_id = update.message.chat_id
                        logger.info(f"Retrieved chat_id from update.message: {chat_id}")
                    elif hasattr(update, 'callback_query') and update.callback_query:
                        chat_id = update.callback_query.message.chat_id
                        logger.info(f"Retrieved chat_id from update.callback_query: {chat_id}")
                    else:
                        logger.error("Could not extract chat_id from update object through any method")
                        return
                except Exception as chat_id_error:
                    logger.error(f"Error extracting chat_id: {str(chat_id_error)}")
                    return
            ...
            if update and hasattr(update, 'message') and update.message:
                try:
                    logger.info("Attempting to send GIF via reply_animation")
                    await update.message.reply_animation(
                        animation=gif_url,
                        caption=WELCOME_MESSAGE,
                        reply_markup=reply_markup,
                ...
                logger.info("No valid update.message to use reply_animation")
            
            # Send via context.bot
            if context and hasattr(context, 'bot'):
                try:
                    logger.info(f"Attempting to send GIF via context.bot to chat_id: {chat_id}")
                    await context.bot.send_animation(
                        chat_id=chat_id,
                        animation=gif_url,
                        caption=WELCOME_MESSAGE,
                ...
                logger.info(f"No valid context.bot to use send_animation - context: {context}")
                
            # Fallback - try with self.bot
            try:
                logger.info(f"Attempting to send GIF via self.bot to chat_id: {chat_id}")
                ...
                logger.info("Successfully sent GIF via self.bot")
                return
            except Exception as e:
                logger.error(f"Error sending GIF via self.bot: {str(e)}")
                logger.error(traceback.format_exc())
            
            # Ultimate fallback - send text message
            try:
                logger.info(f"Attempting to send text message as final fallback to chat_id: {chat_id}")
                if update and hasattr(update, 'message') and update.message:
                    await update.message.reply_text(
                        text=WELCOME_MESSAGE,
                        reply_markup=reply_markup,
                    ...
                    logger.info("Sent text message as fallback via update.message")
                elif chat_id and hasattr(self, 'bot') and self.bot:
                    await self.bot.send_message(
                        chat_id=chat_id,
                        text=WELCOME_MESSAGE,
                        reply_markup=reply_markup,
                    ...
                    logger.info("Sent text message as fallback via self.bot")
                elif context and hasattr(context, 'bot') and context.bot and chat_id:
                    ...
                    logger.info("Sent text message as fallback via context.bot")
                else:
                    logger.error("All message sending methods failed, no valid interfaces available")
            except Exception as text_error:
                logger.error(f"Error sending text message: {str(text_error)}")
                logger.error(traceback.format_exc())
