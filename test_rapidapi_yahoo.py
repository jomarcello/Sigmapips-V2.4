...
# Import RapidAPI Yahoo provider
from trading_bot.services.chart_service.rapidapi_yahoo_provider import RapidapiYahooProvider

async def test_rapidapi_yahoo():
    """Test RapidAPI Yahoo Finance data retrieval directly"""
    logger.info("Starting RapidAPI Yahoo Finance direct test...")
    
    # Create instance of the provider
    rapidapi_provider = RapidapiYahooProvider()
    ...
    for instrument, yf_symbol in test_instruments.items():
        logger.info(f"\nTesting instrument: {instrument} (Yahoo Finance symbol: {yf_symbol})")
        
        try:
            # Get market data
            df, metadata = await rapidapi_provider.get_market_data(instrument)
    ...
    for instrument in test_instruments.keys():
        ...
        try:
            # Generate chart
            chart_bytes = rapidapi_provider.get_chart(instrument, timeframe="1h")
            
            # Save chart to file for inspection
            if chart_bytes:
                output_file = f"test_rapidapi_{instrument.lower()}.png"
...
if __name__ == "__main__":
    # Run the async test
    asyncio.run(test_rapidapi_yahoo()) 