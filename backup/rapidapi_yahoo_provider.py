...
class RapidapiYahooProvider:
    ...
    async def _wait_for_rate_limit():
        ...
        delay = RapidapiYahooProvider._min_delay_between_calls
        
        # Standard rate limiting
        if RapidapiYahooProvider._last_api_call > 0:
            time_since_last_call = current_time - RapidapiYahooProvider._last_api_call
        ...
        RapidapiYahooProvider._last_api_call = time.time()
    ...
    async def _download_data(symbol: str, interval: str = "1d", period: str = "1mo") -> pd.DataFrame:
        ...
        # Wait for rate limit
        await RapidapiYahooProvider._wait_for_rate_limit()
    ...
    async def get_market_data(symbol: str, limit: int = 100) -> Optional[Tuple[pd.DataFrame, Dict]]:
        ...
        try:
            ...
            yahoo_symbol = RapidapiYahooProvider._format_symbol(symbol)
            interval = RapidapiYahooProvider._map_timeframe_to_interval(fixed_timeframe)
            period = RapidapiYahooProvider._map_timeframe_to_period(fixed_timeframe)
            ...
            df = await RapidapiYahooProvider._download_data(
                yahoo_symbol, 
                interval=interval,
                period=period
            )
            ...
            # Validate and clean the data
            df = RapidapiYahooProvider._validate_and_clean_data(df, symbol)
            if df is None:
    ...
    def _format_symbol(instrument: str) -> str:
        ...
        if len(instrument) == 6 and all(c.isalpha() for c in instrument):
            if instrument == "EURUSD":
                ...
                # For other forex pairs, add =X suffix
    ...
    def get_chart(instrument: str, timeframe: str = "1h", fullscreen: bool = False) -> Optional[bytes]:
        ...
        try:
            ...
            yahoo_symbol = RapidapiYahooProvider._format_symbol(instrument)
            
            # Map timeframe to Yahoo Finance interval and period
            interval = RapidapiYahooProvider._map_timeframe_to_interval(timeframe)
            period = RapidapiYahooProvider._map_timeframe_to_period(timeframe)
            
            if not interval:
                logger.error(f"Invalid timeframe: {timeframe}")
                return None
            
            logger.info(f"Getting data for chart: {yahoo_symbol} with interval={interval}, period={period}")
            ...
            try:
                ...
                required_cols = ['Open', 'High', 'Low', 'Close']
                if not all(col in df.columns for col in required_cols):
                    logger.error(f"Missing required columns for {yahoo_symbol}. Available: {df.columns.tolist()}")
                    return None
            ...
            price_precision = RapidapiYahooProvider._get_instrument_precision(instrument)
