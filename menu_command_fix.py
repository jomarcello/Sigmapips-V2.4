    ...
    try:
        ...
        if update and hasattr(update, 'message') and update.message:
            try:
                ...
                logger.info("Menu text sent via update.message.reply_text")
                return True
            except Exception as e:
        ...
        try:
            ...
            logger.info("Menu text sent via bot.send_message")
            return True
        except Exception as e:
            logger.info(f"Failed to send via bot.send_message: {e}")
        
        # Methode 3: Probeer met plain text (zonder HTML parsing)
        try:
            plain_message = "SigmaPips AI Trading Bot\n\nWelkom bij de menu. Selecteer een optie om te beginnen:"
            await bot.send_message(
                chat_id=chat_id,
                text=plain_message,
                reply_markup=reply_markup
            )
            logger.info("Menu text sent via plain text")
            return True
        except Exception as e:
            logger.info(f"Failed to send via plain text: {e}")
...
# Testfunctie om de verbeterde menu_command te integreren
def add_improved_menu_handler(application):
    """Voeg verbeterde menu handler toe aan bestaande application"""
    # Verwijder bestaande menu handlers indien aanwezig
    for i, handler_group in enumerate(application.handlers):
        for j, handler in enumerate(handler_group):
            if isinstance(handler, CommandHandler) and "menu" in handler.commands:
                logger.info(f"Removing existing menu handler: {handler}")
                application.handlers[i].pop(j)
    
    # Voeg nieuwe menu handler toe
    application.add_handler(CommandHandler("menu", improved_menu_command))
    logger.info("Improved menu handler added")
